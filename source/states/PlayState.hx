...
public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true, loop:Bool = false, playOnLoad:Bool = true)
{
				...
				if (notes.length > 0)
				{
					if (startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							{
								if (((!daNote.mustPress && !cpuControlled_opponent && ClientPrefs.data.playOpponent)
									|| (daNote.mustPress && !cpuControlled && !ClientPrefs.data.playOpponent))
									&& !daNote.ignoreNote
									&& !endingSong
	...
	{
		var noteDiff:Float = replayMode ? note.strumTime - time + ClientPrefs.data.ratingOffset : note.strumTime
			- Conductor.songPosition
		...
		for (comboNum in 0...seperatedScore.length)
		{
			...
			if (comboNumTweenScaleY[comboNum] != null)
				comboNumTweenScaleY[comboNum].cancel();
	...
	private function keyPressed(key:Int, ?time:Float = -999999)
	{
		...
		{ // slightly faster than doing `> 0` lol
			...
			if (!ClientPrefs.data.playOpponent)
				goodNoteHit(funnyNote, time);
			else
				opponentNoteHitForOpponent(funnyNote, time);
		}
		else
		{
			var plrInputNotes:Array<Note> = notes.members.filter(function(n:Note):Bool
	...
	public function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if (note != null && guitarHeroSustains && note.parent != null && note.isSustainNote)
		{
			...
			if (parentNote.tail.length > 0)
			{
				...
				if (note == parentNote.tail[0])
					return false; // if you want get 2 miss then detele it
			}
